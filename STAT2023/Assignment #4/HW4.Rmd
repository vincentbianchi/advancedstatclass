...
lm1 <- glm(income_50k~age+workclass+education_num+marital_status+capital_gain+capital_loss+hours_per_week,
...
gam1 <- gam(income_50k~s(age,df=4)+workclass+s(education_num,df=4)+marital_status+s(capital_gain,df=4)+s(capital_loss,df=4)+s(hours_per_week,df=4),
               family='binomial', data=train_data)
...
nn1 <- neuralnet(income_50k=='Yes'~age+workclassLocal_gov+workclassPrivate+workclassSelf_emp_inc+workclassSelf_emp_not_inc+workclassState_gov+education_num+marital_statusMarried_civ_spouse+marital_statusMarried_spouse_absent+marital_statusNever_married+marital_statusSeparated+marital_statusWidowed+capital_gain+capital_loss+hours_per_week, data=x_train_nn, hidden=c(6), linear.output=F)
...
Answer: 
The TP for `lm1`,`gam1`, and `nn1` are 536, 532, and 541 respectfully.
...
Answer: 
The `nn1` model has the best (highest) sensitivity at 0.5861. 
...
Answer:
Based on the lift chart the `gam1` model performs the best in terms of identifying people with income > 50k.
