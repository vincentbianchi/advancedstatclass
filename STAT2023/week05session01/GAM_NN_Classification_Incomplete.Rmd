...
lm_bwd_acc <- confusionMatrix(factor(ifelse(lm_full_pred>0.5, 'Yes', 'No')), test_data$Outcome, positive='Yes') 
```
...
## Fit a GAM
gam1 <- gam(Outcome~s(Age)+s(Experience)+s(Income)+s(Family)+s(CCAvg)+Education+s(Mortgage)+CD_Account+Online+CreditCard, data = train_data, family = 'binomial')
