...
# Remodeling does increase house value
```
...
```{r Q3}
# total value vs year built
plot(total_data$yr_built, total_data$total_value)

# There is a non-linear effect

# total value vs living area
plot(total_data$living_area, total_data$total_value)
...
# Yes we need a log transformation because the distribution is skewed to the right

# Create log_total_value variable
total_data$log_total_value <- log(total_data$total_value+1)

# Histogram of log total value
...
gam1 <- gam(log_total_value~s(lot_sqft,df=4)+s(yr_built,df=4)+s(gross_area,df=4)+s(living_area,df=4)+floors+rooms+full_bath+half_bath+fireplace+remodel,
                data=train_data)
...
# The GAM does capture the non-linear effect of the numerical predictors lot_sqft, yr_built, gross_area, and living_area.
...
# combine with dependent variable Log_total_value
...
We should choose the gam1 model because it has the lowest MAE value (close to nn1 model) and it is simplier than the nn1 model.
...
```{r Q13}
# lm2 
lm2 <- lm(total_value~. - log_total_value, data = train_data)
summary(lm2)

#gam2
gam2 <- gam(total_value~s(lot_sqft,df=4)+s(yr_built,df=4)+s(gross_area,df=4)+s(living_area,df=4)+floors+rooms+full_bath+half_bath+fireplace+remodel,
                data=train_data)
summary(gam2)
```
...
x_train_nn <- cbind.data.frame(train_data$total_value, x_train_nn)
...
set.seed(7)
nn2 <- neuralnet(st_total_value~., data = x_train_nn, hidden = c(4,4)) 
```
