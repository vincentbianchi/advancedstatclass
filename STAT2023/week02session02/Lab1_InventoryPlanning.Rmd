...
# Exercise 1 code
# Scatterplot
plot(total_data$promotion, total_data$sales, ylab='Sales', xlab='Promotion',
     main='Sales v.s. Promotion')

# Boxplot
boxplot(total_data$sales~total_data$promotion)

# The boxplot is more informative
...
#Urban has the highest sales on average
...
hist(total_data$sales, main='Histogram of Sales', xlab='')

# Create log_sales variable
...
round(abs(lm_full$coefficients[4]*100),3)

summary(lm0)$r.squared

round(abs(lm_full$coefficients[4]),3)
...
The $R^2$ of `lm_full` is `r summary(lm_full)$r.squared`

On a day when there is bad weather the log scale price would drop by a factor of -`r round(abs(lm_full$coefficients[4]),3)`. The nominal expected sales (original scale) of the bakery decrease by `r round(abs(lm_full$coefficients[4]*100),3)`%.

The $R^2$ of `lm0` is `r summary(lm0)$r.squared` because you are not evaluating the correlation between any variables because there are no variables.
...
The $R^2$ of `lm_bwd` is `r summary(lm_bwd)$r.squared`. The month variable is removed during the backward selection.
...
# error of lm0
accuracy(lm0_pred, test_data$sales)
...
We should choose the backwards selection model because it has nearly the smallest error across the board and it is the simplest model. Judging by its performance on the test data, it depends on the business context to determine whether the prediction is reasonably accurate.
...
lines(test_data$date[sample_time], lm_bwd_pred[sample_time], col='red')

lines(test_data$date[sample_time], lm0_pred[sample_time], col='blue')
