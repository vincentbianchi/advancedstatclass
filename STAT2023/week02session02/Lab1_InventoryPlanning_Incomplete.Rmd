...
On a day when there is bad weather the nominal expected sales (original scale) of the bakery decrease by `r round(abs(lm_full$coefficients[4]*100),3)`% and would impact the log scale price by a factor of -`r round(abs(lm_full$coefficients[4]),3)`.

The $R^2$ of `lm0` is `r summary(lm0)$r.squared` because you are not evaluating any variables 
